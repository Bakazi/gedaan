...
// ─────────────────────── CONTENT RENDERER ───────────────────────
// contentPageLimit: max number of NEW pages that can be added during content rendering
function drawContent(doc, pal, blocks, startY, contentPageLimit = 99) {
  const margin = 56
  const { width, height } = doc.page
  const usable = width - 2 * margin
  let y = startY
  const isHigh = pal.label.startsWith('BLUEPRINT') || pal.label.startsWith('THINKOVR')
  const isMid = pal.label.startsWith('BLAZE')

  let extraPagesAdded = 0
  const ensure = (need) => {
    if (y + need > height - 60) {
      if (extraPagesAdded >= contentPageLimit) return false // stop adding pages
      doc.addPage()
      extraPagesAdded++
      y = 80
    }
...
async function generateBlueprintPDF({ wish, user, tier }) {
  ...
  // ✅ Page limits per tier (excludes cover page and wish/intro page)
  // spark=2, ignite=4, blaze=6, blueprint_only=10
  const tierPageLimits = { spark: 2, ignite: 4, blaze: 6, blueprint_only: 10 }
  const contentLimit = (tierPageLimits[tierKey] || 2) - 1 // -1 because closing page also counts

  const blocks = parseBlocks(wish.groq_output || '')
  drawContent(doc, pal, blocks, afterWish + 18, contentLimit)
